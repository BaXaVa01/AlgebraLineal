...
from Latex_Funct_Validator import crear_funcion  # Importa tu módulo aquí
...
def toggle_mode():
    ...
    toggle_button.configure(text=f"Cambiar a {'Funciones' if mode == 'Numérico' else 'Numérico'}")
    update_buttons()
...
def update_buttons():
    ...
    if mode == "Funciones":
        # Botones de operadores básicos en formato LaTeX
        ...
        btn_multiply = ctk.CTkButton(button_frame, text=r"\cdot", command=lambda: insert_text(r"\cdot"), width=60)
        btn_divide = ctk.CTkButton(button_frame, text=r"\frac{a}{b}", command=lambda: insert_text(r"\frac{}{}"), width=60)
        ...
        # Botones de funciones avanzadas en formato LaTeX
        btn_sqrt = ctk.CTkButton(button_frame, text=r"\sqrt{x}", command=lambda: insert_text(r"\sqrt{}"), width=60)
        ...
        btn_parentheses.grid(row=1, column=2, padx=5, pady=5)

        # Botones de funciones trigonométricas en formato LaTeX
        btn_sin = ctk.CTkButton(button_frame, text=r"\sin", command=lambda: insert_text(r"\sin{}"), width=60)
        btn_cos = ctk.CTkButton(button_frame, text=r"\cos", command=lambda: insert_text(r"\cos{}"), width=60)
        btn_tan = ctk.CTkButton(button_frame, text=r"\tan", command=lambda: insert_text(r"\tan{}"), width=60)
        btn_log = ctk.CTkButton(button_frame, text=r"\log", command=lambda: insert_text(r"\log{}"), width=60)
