...
def generate_entries():
    try:
        ...
        for i in range(num_vectors):
            tk.Label(vectors_frame, text=f"Escalar para el vector {i+1}:", font=("Arial", 12, "bold")).grid(row=i*2, column=0, sticky="w")
            scalar_entry = tk.Entry(vectors_frame, font=("Arial", 12))
            scalar_entry.grid(row=i*2, column=1, pady=5)
            scalar_entries.append(scalar_entry)

            tk.Label(vectors_frame, text=f"Componentes del vector {i+1}:", font=("Arial", 12, "bold")).grid(row=i*2+1, column=0, sticky="w")
            entries = []
            for j in range(dim):
                entry = tk.Entry(vectors_frame, width=5, font=("Arial", 12))
...
tk.Label(frame, text="Número de vectores:", font=("Arial", 12, "bold")).grid(row=0, column=0, sticky="w")
num_vectors_entry = tk.Entry(frame, font=("Arial", 12))
num_vectors_entry.grid(row=0, column=1, pady=5)

tk.Label(frame, text="Dimensión de los vectores:", font=("Arial", 12, "bold")).grid(row=1, column=0, sticky="w")
dim_entry = tk.Entry(frame, font=("Arial", 12))
dim_entry.grid(row=1, column=1, pady=5)

tk.Button(frame, text="Generar", command=generate_entries, font=("Arial", 12, "bold")).grid(row=1, column=2, padx=10)

vectors_frame = tk.Frame(frame)
vectors_frame.grid(row=2, column=0, columnspan=10, pady=10)

tk.Button(frame, text="Calcular", command=calculate, font=("Arial", 12, "bold")).grid(row=3, column=0, columnspan=3, pady=10)

result_text = tk.StringVar()
result_label = tk.Label(frame, textvariable=result_text, justify="left", anchor="w", font=("Arial", 14, "bold"))
