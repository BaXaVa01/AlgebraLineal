...
class FloatingSideBarUp(ctk.CTkFrame):
    def __init__(self, master, title="Opciones", width=250, height=150, margin_bottom=50, **kwargs):
        super().__init__(master, **kwargs)
        self.master = master
        self.default_height = height
        self.width = width
        self.margin_bottom = margin_bottom  # Espacio extra desde la parte inferior
        self.is_expanded = False  # Estado inicial (retraída)
        ...
        # Botón para expandir/retraer la sidebar
        self.toggle_button = ctk.CTkButton(self.master, text="▲", width=40, command=self.toggle_sidebar, fg_color="gray25")
        self.toggle_button.place(x=10, y=self.master.winfo_height() - self.margin_bottom - 40)

        # Contenido de la sidebar
        self.content_frame = ctk.CTkScrollableFrame(self, fg_color="gray15", corner_radius=8, scrollbar_button_color="gray30")
        self.content_frame.pack(fill="both", expand=True, padx=10, pady=10)
        ...
        # Espacio para los elementos desplazables
        self.scrollable_gif_frame = ctk.CTkFrame(self.content_frame, fg_color="gray10")
        self.scrollable_gif_frame.pack(fill="both", expand=True, pady=10)
        ...
        self.place(x=0, y=self.master.winfo_height(), relwidth=1)
    ...
    def toggle_sidebar(self):
        """Controla la animación para expandir o retraer la sidebar."""
        current_y = self.winfo_y() if self.is_expanded else self.master.winfo_height()
        target_y = (
            self.master.winfo_height() - self.default_height - self.margin_bottom
            if self.is_expanded
    ...
    def update_position(self, event=None):
        ...
        button_y = self.master.winfo_height() - self.margin_bottom - 40
        self.toggle_button.place(x=10, y=button_y)

        # Ajustar la posición de la sidebar
        if self.is_expanded:
            self.place(x=0, y=self.master.winfo_height() - self.default_height - self.margin_bottom, relwidth=1)
        else:
            self.place(x=0, y=self.master.winfo_height(), relwidth=1)

    def add_scrollable_item(self, widget):
        """
        Agrega un widget al contenido desplazable.
        :param widget: Widget que se agregará al scrollable frame.
        """
    ...
    def show_gifs(self, horizontal):
        ...
        for gif_path in gif_files:
            gif_widget = GIFWidget(self.scrollable_gif_frame, gif_paths=[gif_path], width=200, height=200)
            gif_widget.pack(fill="y", pady=5, padx=5)  # Los GIFs estarán organizados verticalmente
